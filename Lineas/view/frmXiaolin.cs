...
{
    public partial class frmXiaolin : Form
    {
        private CXiaolinWu algoritmo;
        private Bitmap bmp;
        private Graphics g;
        private const int ESCALA = 10;
        private List<CXiaolinWu.PixelIntensidad> pixelesLinea;
        ...
        {
            InitializeComponent();
            algoritmo = new CXiaolinWu();
            pixelesLinea = new List<CXiaolinWu.PixelIntensidad>();
            timerSimulacion = new Timer();
            timerSimulacion.Interval = 300;
            timerSimulacion.Tick += TimerSimulacion_Tick;
        }

        private void frmXiaolin_Load(object sender, EventArgs e)
        {
            InicializarLienzo();
        ...
        private void btnDibujar_Click(object sender, EventArgs e)
        {
            ...
            algoritmo.DibujarLinea(g, px1, py1, px2, py2, Color.Purple, 2);

            txtResultado.Text = "=== XIAOLIN WU ===\r\n\r\n" +
                $"({x1},{y1}) -> ({x2},{y2})\r\n" +
                $"Anti-aliasing activo\r\n" +
                $"Gradiente: {algoritmo.Gradiente:F3}";
            pictureBox1.Refresh();
        }
        ...
        private void btnSimular_Click(object sender, EventArgs e)
        {
            ...
            pixelesLinea = algoritmo.CalcularLineaConPasos(px1, py1, px2, py2);
            pasoActual = 0;
            ...
            txtResultado.Text = "=== SIMULACION XIAOLIN WU ===\r\n\r\n" +
                $"Total pixeles: {pixelesLinea.Count}\r\n" +
                $"Gradiente: {algoritmo.Gradiente:F3}\r\n" +
                $"Tipo: {(algoritmo.EsVertical ? "Mas vertical" : "Mas horizontal")}\r\n\r\n" +
                $"Anti-aliasing: 2 pixeles por columna\r\ncon intensidades complementarias";
        ...
        {
            timerSimulacion.Stop();
            for (int i = pasoActual; i < pixelesLinea.Count; i++)
            {
                var p = pixelesLinea[i];
                algoritmo.DibujarPixelConIntensidad(g, p.X, p.Y, Color.Purple, p.Intensidad, ESCALA - 2);
            }
            txtResultado.Text += "\r\n\r\n COMPLETADO!";
            ActualizarEstadoBotones(false);
            pictureBox1.Refresh();
        ...
        {
            if (pasoActual < pixelesLinea.Count)
            {
                var p = pixelesLinea[pasoActual];
                algoritmo.DibujarPixelConIntensidad(g, p.X, p.Y, Color.Purple, p.Intensidad, ESCALA - 2);

                int pasoLogico = pasoActual / 2;
                bool esPrincipal = pasoActual % 2 == 0;

                if (pasoLogico + 1 < algoritmo.PasosSimulacion.Count && esPrincipal)
                {
                    var paso = algoritmo.PasosSimulacion[pasoLogico + 1];
                    txtResultado.Text = $"=== PASO {pasoLogico + 1} ===\r\n\r\n" +
                        $"{paso.Descripcion}\r\n\r\n" +
                        $"Intensidad principal: {paso.IntensidadPrincipal * 100:F0}%\r\n" +
                        $"Intensidad secundario: {paso.IntensidadSecundario * 100:F0}%";
                }
            ...
            {
                timerSimulacion.Stop();
                txtResultado.Text += "\r\n\r\n[OK] LINEA COMPLETADA!\r\nNota el suavizado en los bordes";
                ActualizarEstadoBotones(false);
            }
        }
