...
namespace Algoritmo_de_lineas.Recorte_de_Lineas.view
{
    public partial class frmLiangBarsky : Form
    {
        ...
        private CLiangBarsky algoritmo;
        private Bitmap bmp;
        private Graphics g;
        ...
        private void btnRecortar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string mensaje = $"═══ RESULTADO LIANG-BARSKY ═══\n\n" +
                    $"Línea Original: ({x1}, {y1}) → ({x2}, {y2})\n\n" +
                    $"Parámetros calculados:\n" +
                    $"  dx = {x2 - x1:F2}, dy = {y2 - y1:F2}\n\n";

                if (resultado.ValoresP != null && resultado.ValoresQ != null)
                {
                    mensaje += $"Valores p y q:\n" +
                        $"  p1={resultado.ValoresP[0]:F2}, q1={resultado.ValoresQ[0]:F2} (Izq)\n" +
                        $"  p2={resultado.ValoresP[1]:F2}, q2={resultado.ValoresQ[1]:F2} (Der)\n" +
                        $"  p3={resultado.ValoresP[2]:F2}, q3={resultado.ValoresQ[2]:F2} (Inf)\n" +
                        $"  p4={resultado.ValoresP[3]:F2}, q4={resultado.ValoresQ[3]:F2} (Sup)\n\n";
                }

                mensaje += $"Parámetros de recorte:\n" +
                    $"  t1 = {resultado.T1:F4}\n" +
                    $"  t2 = {resultado.T2:F4}\n\n";
                ...
                if (resultado.LineaVisible)
                {
                    mensaje += $"✓ LÍNEA VISIBLE\n" +
                        $"Línea Recortada:\n" +
                        $"  ({resultado.PuntoInicio.X:F2}, {resultado.PuntoInicio.Y:F2}) →\n" +
                ...
                mensaje += $"Estado: {resultado.Descripcion}";
