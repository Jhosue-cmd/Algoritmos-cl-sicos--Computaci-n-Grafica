...
namespace Algoritmo_de_lineas.Recorte_de_Lineas.view
{
    public partial class frmNichollLeeNicholl : Form
    {
        ...
        private CNichollLeeNicholl algoritmo;
        private Bitmap bmp;
        private Graphics g;
        ...
        {
            InitializeComponent();
            algoritmo = new CNichollLeeNicholl();
        }
        ...
        private void btnRecortar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string mensaje = $"═══ RESULTADO NICHOLL-LEE-NICHOLL ═══\n\n" +
                    $"Línea Original: ({x1}, {y1}) → ({x2}, {y2})\n\n" +
                    $"Análisis de P1:\n" +
                    $"  Región: {resultado.RegionDescripcion}\n" +
                    $"  Código: {resultado.RegionPuntoInicio}\n\n";
                ...
                mensaje += $"Estado: {resultado.Descripcion}\n\n" +
                    $"Ventaja NLN: Reduce operaciones\n" +
                    $"de recorte al analizar regiones.";
