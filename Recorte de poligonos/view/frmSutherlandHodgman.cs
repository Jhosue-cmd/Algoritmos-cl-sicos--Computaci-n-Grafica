...
namespace Algoritmo_de_lineas.Recorte_de_poligonos.view
{
    public partial class frmSutherlandHodgman : Form
    {
        ...
        private CSutherlandHodgman algoritmo;
        private Bitmap bmp;
        private Graphics g;
        ...
        {
            InitializeComponent();
            algoritmo = new CSutherlandHodgman();
        ...
        private void btnRecortar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // Dibujar polígono original (semitransparente)
                algoritmo.DibujarPoligono(g, verticesPoligono, centroX, centroY, ESCALA,
                    Color.LightGray, Color.LightGray, true);

                // Dibujar polígono recortado
                if (resultado.PoligonoVisible)
                {
                    algoritmo.DibujarPoligono(g, resultado.PoligonoRecortado, centroX, centroY, ESCALA,
                ...
                // Mostrar resultado
                string mensaje = $"═══ RESULTADO SUTHERLAND-HODGMAN ═══\n\n" +
                    $"Polígono Original:\n" +
                    $"  Vértices: {resultado.VerticesOriginales}\n\n" +
                    $"Ventana de Recorte:\n" +
                    $"  ({xMin}, {yMin}) - ({xMax}, {yMax})\n\n";
                ...
                if (resultado.PoligonoVisible)
                {
                    mensaje += $"✓ POLÍGONO VISIBLE\n" +
                        $"  Vértices resultantes: {resultado.VerticesResultantes}\n\n" +
                        $"Coordenadas del polígono recortado:\n";

                    for (int i = 0; i < resultado.PoligonoRecortado.Count; i++)
                    {
                        var v = resultado.PoligonoRecortado[i];
        ...
        private void btnPoligonoEjemplo_Click(object sender, EventArgs e)
        {
            ...
            // Crear un polígono de ejemplo (pentágono)
            verticesPoligono.Add(new PointF(-12, 0));
            verticesPoligono.Add(new PointF(-4, -10));
            verticesPoligono.Add(new PointF(8, -6));
            verticesPoligono.Add(new PointF(10, 5));
            ...
            RedibujarEscena();
            lblEstado.Text = "Polígono de ejemplo cargado. Presione Recortar.";
        }
        ...
        private void pictureBox1_MouseClick(object sender, MouseEventArgs e)
        {
            if (chkDibujarConMouse.Checked)
            {
                ...
                float x = (e.X - centroX) / (float)ESCALA;
                float y = (centroY - e.Y) / (float)ESCALA;

                // Redondear a enteros para simplicidad
        ...
        private void RedibujarEscena()
        {
            ...
            // Dibujar ventana si hay datos válidos
            ...
            {
                algoritmo.DibujarPoligono(g, verticesPoligono, centroX, centroY, ESCALA,
                    Color.Green, Color.LightGreen, true);
            }
        ...
        private bool ValidarDatosVentana()
        {
            ...
            if (!float.TryParse(txtXMin.Text, out float xMin) ||
                ...
                !float.TryParse(txtYMax.Text, out float yMax))
            {
                MessageBox.Show("Los valores de la ventana deben ser números válidos.",
                    "Datos inválidos", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
