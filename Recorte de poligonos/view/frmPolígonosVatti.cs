...
{
    public partial class frmVatti : Form
    {
        private CVatti algoritmo;
        private Bitmap bmp;
        private Graphics g;
        ...
        {
            InitializeComponent();
            algoritmo = new CVatti();
            verticesPoligono = new List<PointF>();
        }

        private void frmVatti_Load(object sender, EventArgs e)
        {
            InicializarLienzo();
            txtXMin.Text = "-8";
            txtYMin.Text = "-8";
            txtXMax.Text = "8";
            txtYMax.Text = "8";
            cmbOperacion.Items.AddRange(new object[] { "Intersección", "Unión", "Diferencia", "XOR" });
        ...
        private void btnAgregarVertice_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Ingrese las coordenadas X e Y.", "Datos incompletos", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
        ...
        private void btnRecortar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Agregue al menos 3 vértices.", "Polígono incompleto", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                ...
                CVatti.TipoOperacion operacion = (CVatti.TipoOperacion)cmbOperacion.SelectedIndex;
                var resultado = algoritmo.Recortar(verticesPoligono, operacion);
                ...
                string mensaje = $"═══ RESULTADO VATTI ═══\n\nOperación: {cmbOperacion.SelectedItem}\n" +
                    $"Vértices originales: {resultado.VerticesOriginales}\n\n";
                if (resultado.PoligonoVisible)
                    mensaje += $"✓ RESULTADO VISIBLE\nPolígonos: {resultado.PoligonosResultantes}\n";
                else
                    mensaje += "✗ SIN RESULTADO VISIBLE\n";
                mensaje += $"\n{resultado.Descripcion}";

                txtResultado.Text = mensaje;
                lblEstado.Text = resultado.PoligonoVisible ? "Operación completada" : "Sin resultado visible";
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Error: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void btnPoligonoEjemplo_Click(object sender, EventArgs e)
        {
            ...
            verticesPoligono.Add(new PointF(-3, 4));
            verticesPoligono.Add(new PointF(-12, 4));
            verticesPoligono.Add(new PointF(-5, -2));
            verticesPoligono.Add(new PointF(-7, -10));
            verticesPoligono.Add(new PointF(0, -5));
            verticesPoligono.Add(new PointF(7, -10));
            verticesPoligono.Add(new PointF(5, -2));
            verticesPoligono.Add(new PointF(12, 4));
            verticesPoligono.Add(new PointF(3, 4));
            ...
            RedibujarEscena();
            lblEstado.Text = "Polígono estrella cargado. Presione Recortar.";
        }
        ...
        private void RedibujarEscena()
        {
            ...
            else if (verticesPoligono.Count > 0)
            {
                using (Brush brush = new SolidBrush(Color.Green))
                    foreach (var v in verticesPoligono)
                        g.FillEllipse(brush, centroX + v.X * ESCALA - 4, centroY - v.Y * ESCALA - 4, 8, 8);
            }
            pictureBox1.Refresh();
        }
        ...
        private bool ValidarDatosVentana()
        {
            ...
            {
                MessageBox.Show("Ingrese todos los valores de la ventana.", "Datos incompletos", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return false;
            }
            if (!float.TryParse(txtXMin.Text, out float xMin) || !float.TryParse(txtYMin.Text, out float yMin) ||
                !float.TryParse(txtXMax.Text, out float xMax) || !float.TryParse(txtYMax.Text, out float yMax))
            {
                MessageBox.Show("Los valores deben ser números válidos.", "Datos inválidos", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
            if (!algoritmo.ValidarVentana(xMin, yMin, xMax, yMax))
            {
                MessageBox.Show("XMin < XMax y YMin < YMax.", "Ventana inválida", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
            return true;
        }
