...
{
    public partial class frmPatternFill : Form
    {
        #region Campos privados

        private CPatternFill algoritmo;
        private Bitmap bmp;
        private Graphics g;
        ...
        public frmPatternFill()
        {
            InitializeComponent();
            algoritmo = new CPatternFill();
        ...
        private void frmPatternFill_Load(object sender, EventArgs e)
        {
            InicializarLienzo();
            CargarPatrones();
            cmbColorPatron.SelectedIndex = 1;
            cmbColorFondo.SelectedIndex = 0;
            cmbColorBorde.SelectedIndex = 2;
            numTamanoPatron.Value = 8;
        }
        ...
        private void CargarPatrones()
        {
            ...
            cmbPatron.Items.Add("Líneas Horizontales");
            cmbPatron.Items.Add("Líneas Verticales");
            cmbPatron.Items.Add("Líneas Diagonales (/)");
            cmbPatron.Items.Add("Líneas Diagonales (\\)");
            cmbPatron.Items.Add("Cuadrícula");
            cmbPatron.Items.Add("Puntos");
            cmbPatron.Items.Add("Cruces");
            cmbPatron.Items.Add("Tablero de Ajedrez");
        ...
        private void btnRellenar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // Validar tamaño del patrón
                int tamanoPatron = (int)numTamanoPatron.Value;
                if (!algoritmo.ValidarTamanoPatron(tamanoPatron))
                {
                    MessageBox.Show("El tamaño del patrón debe estar entre 1 y 50.",
                        "Tamaño inválido", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }

                // Configurar algoritmo
                algoritmo.TamanoPatron = tamanoPatron;

                // Obtener tipo de patrón
                CPatternFill.TipoPatron tipoPatron = (CPatternFill.TipoPatron)cmbPatron.SelectedIndex;

                // Obtener colores
                Color colorPatron = ObtenerColorSeleccionado(cmbColorPatron);
                Color colorFondo = chkFondoTransparente.Checked ? Color.Transparent : ObtenerColorSeleccionado(cmbColorFondo);
                ...
                // Dibujar el polígono con patrón
                algoritmo.DibujarRellenoPatron(g, vertices, tipoPatron, colorPatron, colorFondo, colorBorde);
                ...
                MessageBox.Show($"Polígono rellenado con patrón correctamente.\n\n" +
                    $"Número de vértices: {vertices.Count}\n" +
                    $"Área aproximada: {area:F2} unidades²\n" +
                    $"Perímetro aproximado: {perimetro:F2} unidades\n" +
                    $"Patrón: {cmbPatron.SelectedItem}\n" +
                    $"Tamaño del patrón: {tamanoPatron}px\n\n" +
                    $"Algoritmo: Pattern Fill",
                    "Éxito", MessageBoxButtons.OK, MessageBoxIcon.Information);

                lblEstado.Text = "Relleno con patrón completado.";
            }
            catch (Exception ex)
            {
        ...
        private void cmbPatron_SelectedIndexChanged(object sender, EventArgs e)
        {
            ActualizarVistaPrevia();
        }

        private void numTamanoPatron_ValueChanged(object sender, EventArgs e)
        {
            ActualizarVistaPrevia();
        }

        private void cmbColorPatron_SelectedIndexChanged(object sender, EventArgs e)
        {
            ActualizarVistaPrevia();
        }

        private void cmbColorFondo_SelectedIndexChanged(object sender, EventArgs e)
        {
            ActualizarVistaPrevia();
        }

        private void chkFondoTransparente_CheckedChanged(object sender, EventArgs e)
        {
            cmbColorFondo.Enabled = !chkFondoTransparente.Checked;
            ActualizarVistaPrevia();
        }
        ...
        {
            if (pnlVistaPrevia.Width <= 0 || pnlVistaPrevia.Height <= 0)
                return;

            try
            {
                using (Bitmap bmpPrevia = new Bitmap(pnlVistaPrevia.Width, pnlVistaPrevia.Height))
                using (Graphics gPrevia = Graphics.FromImage(bmpPrevia))
                {
                    gPrevia.Clear(Color.White);

                    algoritmo.TamanoPatron = (int)numTamanoPatron.Value;
                    CPatternFill.TipoPatron tipoPatron = (CPatternFill.TipoPatron)cmbPatron.SelectedIndex;
                    Color colorPatron = ObtenerColorSeleccionado(cmbColorPatron);
                    Color colorFondo = chkFondoTransparente.Checked ? Color.LightGray : ObtenerColorSeleccionado(cmbColorFondo);

                    algoritmo.DibujarVistaPrevia(gPrevia, 5, 5, pnlVistaPrevia.Width - 10,
                        pnlVistaPrevia.Height - 10, tipoPatron, colorPatron, colorFondo);

                    pnlVistaPrevia.BackgroundImage?.Dispose();
                    pnlVistaPrevia.BackgroundImage = (Bitmap)bmpPrevia.Clone();
                }
            }
            catch
            {
                // Ignorar errores de vista previa
            }
        }
        ...
        private Color ObtenerColorSeleccionado(ComboBox cmb)
        {
            ...
            {
                case 0: return Color.FromArgb(200, 200, 255); // Azul Claro
